 ...
 * CookieManager.hpp                                      *
 *                                                        *
 * hprose cookie manager unit for cpp.                    *
 *                                                        *
...
#ifndef HPROSE_CLIENT_COOKIE_MANAGER_HPP
#define HPROSE_CLIENT_COOKIE_MANAGER_HPP
...
struct cookie {
    ...
    inline std::string rawform() const {
        return name + "=" + value;
    };
...
public:
    ...
    inline static CookieManager * SharedInstance() {
...
public:
    ...
    std::string GetCookie(const std::string & host, const std::string & path, bool secure = false) {
    ...
    void SetCookie(const std::string & host, std::string content) {
            ...
            if (pos != std::string::npos) {
                ...
                while (true) {
                    ...
                    if (_strcmpi(key.c_str(), "path") == 0) {
                        ...
                        if (!value.empty()) {
                            c.expiry = parseRFCDatetime(value);
                        }
                    }
                    if (eof) break;
                }
...
private:
    ...
    AllCookies container;
    boost::shared_mutex mutex; // read-write lock for containers

}; // class CookieManager

} // namespace hprose

#endif // HPROSE_CLIENT_COOKIE_MANAGER_HPP
