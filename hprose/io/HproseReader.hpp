...
#include <hprose/io/HproseTags.hpp>
#include <hprose/io/HproseTypes.hpp>
...
public:
    ...
    bool ReadBool() {
        switch (int tag = stream.get()) {
            ...
            case HproseTags::TagLong: return ReadUntil(HproseTags::TagSemicolon).str() != "0";
    ...
    ReturnType ReadString() {
        switch (int tag = stream.get()) {
            ...
            case HproseTags::TagDouble: return Stringify<ReturnType>(ReadUntil(HproseTags::TagSemicolon).str());
            case HproseTags::TagNull:
            case HproseTags::TagEmpty: return ReturnType();
...
private:
    ...
    std::stringstream ReadUntil(char tag) {
        std::stringstream ss;
        char c = stream.get();
        while ((c != tag) && (c != -1)) {
            ss << c;
